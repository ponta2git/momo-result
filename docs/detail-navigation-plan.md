...
両画面とも本文を読み終えたページ下部の一か所へ配置する。試合は結果・メモ・記録情報の後、開催は開催内容の後とする。リンクと端の非link表示は文字色・余白・各枠内の中央揃えを共通化し、状態による位置のずれを防ぐ。前後は「前の試合／後の試合」「前の開催／次の開催」とし、存在しない側は理由のある非link表示にする。矢印だけや`#`へのlinkにしない。
