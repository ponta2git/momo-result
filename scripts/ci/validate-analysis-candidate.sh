...
jq -ers \
  ...
  --arg runId "${expected_run_id}" '
    ...
    (.imageArtifactId | type == "string" and test("^[1-9][0-9]*\\z")) and
    (.imageArtifactDigest | type == "string" and test("^sha256:[0-9a-f]{64}\\z")) and
    (.tarSha256 | type == "string" and test("^[0-9a-f]{64}\\z")) and
    (.configSha256 | type == "string" and test("^[0-9a-f]{64}\\z"))) |
    "candidate_sha=\(.commit)",
    "candidate_run_attempt=\(.runAttempt)",
    "config_sha256=\(.configSha256)",
    "image_artifact_digest=\(.imageArtifactDigest)",
    "image_artifact_id=\(.imageArtifactId)",
    "image_artifact_name=\(.imageArtifactName)",
    "image_ref=\(.imageRef)",
