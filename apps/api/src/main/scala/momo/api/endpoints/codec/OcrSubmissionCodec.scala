...
object OcrSubmissionCodec:
  def command(id: String, request: PutOcrSubmissionRequest): Either[AppError, PutOcrSubmissionCommand] = for
    ...
    members <- request.members.traverse { member => for
      screen <- ScreenType.fromExplicitWire(member.screenType).toRight(
        AppError.ValidationFailed("Choose an explicit OCR screen type."))
      _ <- Either.cond(member.uploadIdempotencyKey.matches("[A-Za-z0-9_-]{16,128}"), (),
        AppError.ValidationFailed("uploadIdempotencyKey must contain 16 to 128 safe characters."))
    yield OcrSubmissionMember(screen,
      SourceImageIdempotencyHash.fromRawKey(member.uploadIdempotencyKey).value,
      member.imageSha256, member.imageByteLength)
    }
