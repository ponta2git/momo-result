...
`OCR_TIMEOUT_SECONDS` はOCR認識呼び出しの timeout、`OCR_REDIS_CLAIM_IDLE_SECONDS` はRedis PEL回収の待機時間であり、同じ値として扱わない。claim idle は正当な長時間ジョブを早期に重複配送しないよう、API stale job reaper の基準値以上にする。
