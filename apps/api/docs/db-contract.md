...
業務要件は `requirements/base.md` §5（試合結果） / §8.3（事件簿 6 項目固定） を参照。
...
- **`members` の seed**: 固定 4 名 (`ponta / akane-mami / otaka / eu`) で進めているが、momo-db の seed が未提供。Doobie 接続フェーズで `members` への seed 追加可否を確認する。
- **`game_titles` / `map_masters` / `season_masters` の初期データ投入**: マスタ管理 UI 経由 / migration 経由 のどちらにするか後続で判断。
- **試合の論理削除**: 物理削除（cascade）で進めるが、CSV 出力時に削除済みを除外する仕様にするか別途検討。
- **CSV 出力の権限**: ログイン中 4 名すべてに許可。RLS は MVP では使用せず、API 層で session を確認する。
- **server-side session の有効期限削除ジョブ**: 後続スライス。

---

## スキーマ変更フロー

1. 仕様変更を `requirements/base.md` または `requirements/system-design.md` に記述。
