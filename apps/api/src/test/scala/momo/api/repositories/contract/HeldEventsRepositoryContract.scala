...
trait HeldEventsRepositoryContract:
  ...
  test("find returns None for an unknown id"):
    freshRepo.flatMap(_.find("does_not_exist")).map(r => assertEquals(r, None))

  test("create + find round-trips the held event"):
    val event = HeldEvent("held_2026_04_30", baseInstant)
    for
      repo <- freshRepo
      _ <- repo.create(event)
  ...
  test("list orders events by heldAt desc, then by id desc as tie-breaker"):
    val older = HeldEvent("held_alpha", at(0))
    val newer = HeldEvent("held_beta", at(60))
    val tieA = HeldEvent("held_zzz", at(120))
    val tieB = HeldEvent("held_aaa", at(120))
    for
      repo <- freshRepo
      _ <- repo.create(older)
      _ <- repo.create(newer)
    ...
    yield
      ...
      assertEquals(list.map(_.id).take(2).toSet, Set("held_zzz", "held_aaa"))
      assertEquals(list(2).id, "held_beta")
      assertEquals(list(3).id, "held_alpha")

  test("list applies limit"):
    val a = HeldEvent("held_001", at(0))
    val b = HeldEvent("held_002", at(60))
    val c = HeldEvent("held_003", at(120))
    for
      repo <- freshRepo
      _ <- repo.create(a)
      _ <- repo.create(b)
  ...
  test("list with negative limit returns no events"):
    val event = HeldEvent("held_neg", baseInstant)
    for
      repo <- freshRepo
      _ <- repo.create(event)
  ...
  test("list with empty / whitespace query is treated as no filter"):
    val a = HeldEvent("held_alpha", at(0))
    val b = HeldEvent("held_beta", at(60))
    for
      repo <- freshRepo
      _ <- repo.create(a)
      _ <- repo.create(b)
    ...
    yield
      assertEquals(empty.map(_.id).toSet, Set("held_alpha", "held_beta"))
      assertEquals(blank.map(_.id).toSet, Set("held_alpha", "held_beta"))

  test("list query filters by case-insensitive substring of id"):
    val a = HeldEvent("held_2026_04_30", at(0))
    val b = HeldEvent("held_2026_05_07", at(60))
    val c = HeldEvent("held_2025_12_25", at(120))
    for
      repo <- freshRepo
      _ <- repo.create(a)
      _ <- repo.create(b)
    ...
    yield
      assertEquals(lower.map(_.id).toSet, Set("held_2026_04_30", "held_2026_05_07"))
      assertEquals(upper.map(_.id).toSet, Set("held_2026_04_30", "held_2026_05_07"))
