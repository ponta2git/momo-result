...
final class CreateOcrJob[F[_]: MonadThrow](
  ...
  def run(command: CreateOcrJobCommand): F[Either[AppError, CreatedOcrJob]] =
    (for
      screenType <- EitherT.fromEither[F](
        ScreenType
          .fromWire(command.requestedImageType)
          .toRight(
      ...
      image <- EitherT(
        imageStore
          .find(imageId)
          .map(_.toRight(AppError.NotFound("image", command.imageId)))
      )
