...
目的: サブシステムごとのテスト対象、テストサイズ、coverage 管理、CI成果物を一枚で確認できるようにする。
...
Phase 1 では coverage を gate にしない。CIで report を生成し、artifact と summary に残す。
...
| Subsystem | Metric | 参考 raw | 丸め候補 |
|---|---|---:|---:|
| web | statements | 83.46% | 80% |
| web | branches | 77.49% | 75% |
| web | functions | 82.58% | 80% |
| web | lines | 83.92% | 80% |
| api | statements | 78.8% | 75% |
| api | branches | 71.2% | 70% |
| api | lines | 79.0% | 75% |
| ocr-worker | total | 89.1% | 85% |
| ocr-worker | lines | 91.2% | 90% |
| ocr-worker | branches | 80.4% | 80% |
...
| `src/shared/api` | S / M | API wrapper、Problem Details、query key、cache helper。branch 90%目安、重要ファイルは100%。 |
| `src/shared/auth`, `src/shared/lib`, `src/shared/domain` | S | pure logic とブラウザ境界。分岐80-90%目安。 |
...
| 対象範囲 | 主テストサイズ | 確保するcoverage / oracle |
|---|---|---|
| `domain` | S | 不変条件、lifecycle、policy。branch 90%目安。 |
| `usecases` | S / M | 状態遷移、validation、副作用境界。DTO、DB row intent、queue payloadをassertする。 |
| `endpoints`, `endpoints/codec` | S / M | request / response roundtrip、OpenAPI、Problem Details。 |
...
| `features/screen_detection`, `features/player_order` | S | screen type、色順、fallback条件。branch 85-95%目安。 |
...
現行 coverage 設定は `apps/ocr-worker/pyproject.toml` を正とする。OCR精度劣化は code coverage では検知しにくいため、Phase 1 後に accuracy report のartifact化を別途検討する。
...
Phase 1 の目的は、coverage report をCIに組み込み、artifact と job summary として残すこと。coverage値ではPRを落とさない。
