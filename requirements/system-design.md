...
- DBは summit アプリと共有する Neon PostgreSQL を使う。
- 本アプリは summit の `held_events` や `members` などをDB契約として参照する。

### 7.2 マイグレーション

- MVPではDBマイグレーションの正本を summit 側Drizzleマイグレーションに置く。
- このアプリはDB契約としてスキーマを参照する。
- DBスキーマ変更が必要なリリースでは、summit側でマイグレーションを適用した後に、このアプリをデプロイする。
...
- マルチステージDockerfileで web/api/ocr-worker をビルドする。
...
これらはTesseract + OpenCV/Pillowでの実測後に決定する。
