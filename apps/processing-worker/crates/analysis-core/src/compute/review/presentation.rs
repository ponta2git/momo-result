    ...
    json!({
        ...
        "evidence": [
            ...
            },
            {
                ...
                "method": if candidate.confidence_low.is_some() { Some("event_cluster_bootstrap_v1") } else { None },
                "confidenceLow": candidate.confidence_low.map(|value| round(value, 6)),
                "confidenceHigh": candidate.confidence_high.map(|value| round(value, 6)),
                "stability": candidate.event_stability.map(|value| round(value, 6)),
