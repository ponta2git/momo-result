...
final class InMemoryMemberAliasesRepository[F[_]: Sync] private (ref: Ref[F, List[MemberAlias]])
  ...
  override def create(alias: MemberAlias): F[Either[AppError, Unit]] = ref.modify { aliases =>
  ...
  override def update(alias: MemberAlias): F[Either[AppError, Unit]] = ref.modify { aliases =>
  ...
  override def delete(id: MemberAliasId): F[Either[AppError, Unit]] = ref.modify { aliases =>
