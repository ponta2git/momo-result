...
- 初回 route loading と feature loading は、pathname と安全化済み query から確定する page width、専用 shell、戻る導線、header の eyebrow / description、常設 action、header と surface 間の文脈 slot を ready と同じ順序で保持する。data 取得結果で初めて有無が決まる action や notice は推測して skeleton を足さず、確定済み slot の大きさと responsive な積み替えだけを保つ。状態ごとに似た余白を個別再現せず、同じ layout primitive と親 composition を owner にする。
