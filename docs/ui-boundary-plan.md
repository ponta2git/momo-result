...
| [AdminAccountRow](../apps/web/src/features/adminAccounts/AdminAccountRow.tsx) | 固定された名前のセルが独自の不透明なsurface色を持つ。行背景を追加するだけでは対象名まで帯がつながらない | 固定セルも共有の行表示に接続。不透明性を保って横scroll時の内容の透けを防ぐ |
...
- 固定見出しセルが独自の白背景を持つ管理のアカウント行は、Cと同時に修正する。DataTable側が決める通常／hover／focus時の行の色を固定セルも使い、個別セルへ別の時間・状態selectorを複製しない。単純な透明化では横scroll時に背後の文字が透けるため、不透明な面を維持する接続を先行検証する。対象名に届かない帯を「適用済み」にしない。
...
| 行に薄い背景があれば対象を追える | 前案では不十分。管理の固定名前セルが白背景を持つことをコードとChromeで確認。追加する帯を対象名まで接続する設計へ修正 |
...
今回確認したのは、既存規約・共有部品・利用箇所のコード、およびChrome上の管理一覧の固定名前セルの不透明な背景である。新しい色・行focusの完成表示と、その利用者への効果は未検証であり、以下の実施段階で確かめる。
