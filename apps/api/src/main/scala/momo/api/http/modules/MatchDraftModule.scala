...
object MatchDraftModule:
  def routes[F[_]: Async](
    ...
    MatchDraftEndpoints.create.serverLogic {
      case (devUser, csrfToken, idemKey, request) =>
        security.authorizeMutation(devUser, csrfToken) { member =>
          IdempotencyHandler.wrap[F, CreateMatchDraftRequest, MatchDraftResponse](
            ...
            MatchDraftCodec.parseInstantOption[F](request.playedAt).flatMap {
              ...
              case Right(playedAt) =>
                security.respond(createMatchDraft.run(
                  MatchDraftCodec.toCreateCommand(request, playedAt),
                  member.memberId,
    ...
    MatchDraftEndpoints.update.serverLogic {
      case (draftId, devUser, csrfToken, idemKey, request) =>
        security.authorizeMutation(devUser, csrfToken) { member =>
    ...
    MatchDraftEndpoints.get.serverLogic { case (draftId, devUser) =>
      security.authorizeRead(devUser) { member =>
        security.respond(getMatchDraft.run(MatchDraftId(draftId), member.memberId))(
    ...
    MatchDraftEndpoints.cancel.serverLogic { case (draftId, devUser, csrfToken) =>
      security.authorizeMutation(devUser, csrfToken) { member =>
        security.respond(cancelMatchDraft.run(MatchDraftId(draftId), member.memberId))(_ =>
          CancelMatchDraftResponse(matchDraftId = draftId, status = "cancelled")
        )
      }
    },
    MatchDraftEndpoints.listSourceImages.serverLogic { case (draftId, devUser) =>
      security.authorizeRead(devUser) { member =>
        security.respond(getMatchDraftSourceImages.list(MatchDraftId(draftId), member.memberId))(
