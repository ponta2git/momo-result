...
object MaxBodySizeMiddleware:
  ...
  def uploadOnly[F[_]: Async](limitBytes: Long)(http: HttpApp[F]): HttpApp[F] =
    Kleisli { request =>
      if isUpload(request) then applyLimit(request, limitBytes, http)
  ...
  private def applyLimit[F[_]: Async](
    ...
    request.headers.get[`Content-Length`].map(_.length) match
  ...
  private def limitStream[F[_]: Async](body: fs2.Stream[F, Byte], limitBytes: Long) = body.chunks
    .evalMapAccumulate(0L) { (seen, chunk) =>
      ...
      if next > limitBytes then Async[F].raiseError[(Long, Chunk[Byte])](
        RequestBodyTooLarge(limitBytes)
      )
      else (next, chunk).pure[F]
    }
    .flatMap { case (_, chunk) => fs2.Stream.chunk(chunk) }

  private def isUpload[F[_]](request: Request[F]): Boolean =
    request.method.name == "POST" && request.uri.path.renderString == "/api/uploads/images"

  private def problem[F[_]: Async](limitBytes: Long): F[Response[F]] =
    val (status, body) = ProblemDetails.from(AppError.PayloadTooLarge(
      s"Upload request must be ${limitBytes.toString} bytes or smaller."
    ))
