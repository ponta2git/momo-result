...
object UploadImageResponse:
  def from(image: StoredImage): UploadImageResponse =
    UploadImageResponse(
...
final case class CreateOcrJobResponse(
    jobId: String,
    draftId: String,
    status: String
) derives Codec.AsObject
...
object OcrJobResponse:
  def from(job: OcrJob): OcrJobResponse =
    OcrJobResponse(
      jobId = job.id.value,
      ...
      failure =
        job.failure.map(f => OcrFailureResponse(f.code.wire, f.message, f.retryable, f.userAction)),
...
object OcrDraftResponse:
  def from(draft: OcrDraft): OcrDraftResponse =
    OcrDraftResponse(
