    ...
    val program = for
      ...
      result <- existing match
        case Some(saved) =>
          ...
          else Left(AppError.Conflict("This submissionId belongs to another request.")).pure[ConnectionIO]
        case None => admit(submission)
    yield result
    program.transact(transactor).flatTap(_ => wake(transactor))

  private def admit(submission: OcrSubmission): ConnectionIO[Either[AppError, OcrSubmission]] = for
    draft <- sql"""SELECT status FROM match_drafts WHERE id = ${submission.matchDraftId} FOR UPDATE"""
      .query[String].option
    ...
    result <-
      if draft.isEmpty then
        ...
        Left(AppError.TooManyRequests("Too many unfinished OCR submissions. Retry after they finish.")).pure[ConnectionIO]
      else for
        ...
        admitted <-
          if usable.contains(false) then
            Left(AppError.Conflict("An image from this request is no longer available.")).pure[ConnectionIO]
          else insert(submission).as(Right(submission))
      yield admitted
  yield result
...
private[api] object PostgresOcrSubmissions:
  private final case class Header(
      id: String, owner: AccountId, draftId: MatchDraftId, hints: String, status: String,
      deadline: Instant, createdAt: Instant, finishedAt: Option[Instant],
  )
      ...
      .query[Header].option.flatMap(_.traverse { header =>
        for
          hints <- decode[OcrJobHints](header.hints).left.map(_ =>
            PostgresDataIntegrityException.inconsistentRow("ocr_submissions", id, "invalid reading conditions")
          ).liftTo[ConnectionIO]
        ...
        yield OcrSubmission(header.id, header.owner, header.draftId, hints, header.status,
          header.deadline, header.createdAt, header.finishedAt, members)
      })
    ...
    if ids.isEmpty then ().pure[ConnectionIO]
      ...
      sql"""SELECT id FROM ocr_submissions
        ...
        .query[String].to[List].flatMap { submissions =>
          ...
          else sql"""UPDATE ocr_submissions SET status = 'aborted', finished_at = GREATEST($now, clock_timestamp(), created_at)
