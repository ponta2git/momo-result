...
function NavItemLink({ item }: { item: GlobalNavItem }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          "inline-flex min-h-11 min-w-11 shrink-0 items-center justify-center gap-2 rounded-sm border px-3 py-2 text-sm font-semibold lg:min-h-9 lg:min-w-0 lg:py-1",
          isActive
      ...
      >
        <div className="flex min-w-0 items-center justify-between gap-2 lg:contents">
          <div className="flex min-w-0 items-center gap-2 lg:col-start-1 lg:row-start-1">
            <Link
              className="-ml-1 inline-flex min-h-11 items-center rounded-xs px-1 py-1 text-sm font-semibold text-[var(--color-text-primary)] hover:text-[var(--color-action)] lg:min-h-9"
              to={brandTo}
            >
              {brandLabel}
            </Link>
