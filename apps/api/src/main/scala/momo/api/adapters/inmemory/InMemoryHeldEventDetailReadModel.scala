  ...
  override def find(id: HeldEventId): F[Option[HeldEventDetail]] =
    events.find(id).flatMap(_.traverse { event =>
      for
        ...
        labels <- confirmed.traverse(m =>
          metadata.labels(Some(m.gameTitleId), Some(m.seasonMasterId), Some(m.mapMasterId))
        )
      yield HeldEventDetail(
        event,
        confirmed.zip(labels).map { case (m, label) =>
          HeldEventDetail.Match(
            m.id,
            m.matchNoInEvent,
            m.gameTitleId,
            m.seasonMasterId,
            ...
            m.mapMasterId,
            m.playedAt,
            m.note.body.map(_.value),
            m.players.byPlayOrder.map(p =>
              HeldEventDetail.Player(
                p.memberId,
                p.playOrder,
                p.rank,
                p.totalAssetsManYen,
        ...
        drafts.flatMap(d =>
          (d.matchDraftId, momo.api.domain.MatchDraftStatus.fromWire(d.status)).mapN {
            (draftId, status) =>
              HeldEventDetail.Draft(
                draftId,
                status,
                d.matchNoInEvent,
                d.gameTitleId,
