...
pub(super) fn fit<const N: usize>(observations: &mut [Observation<N>]) -> Result<Fit<N>, ()> {
    if N == 0
    ...
    // Equal observations have identical feature/outcome bits, so stable ordering adds no
    // semantics. Sort in place without an auxiliary observation buffer; callers may reuse it.
    observations.sort_unstable_by(compare_observations);
