...
final class ProblemDetailsSpec extends FunSuite:
  test("internal errors expose a generic public detail"):
    ...
    val problem = ProblemDetails.from(AppError.Internal(s"stored response leaked $secret")).body
  ...
  test("validation errors do not expose internal field names"):
    val problem = ProblemDetails.from(AppError.ValidationFailed("matchNo must be positive.")).body
  ...
  test("not-found errors do not expose resource names or identifiers"):
    val problem = ProblemDetails.from(AppError.NotFound("match draft", "draft-secret")).body
