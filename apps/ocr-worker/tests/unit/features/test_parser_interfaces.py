...
def test_category_parser_returns_payload_with_profile_context(tmp_path: Path) -> None:
    ...
    payload = parse_payload(parser, context)
...
def test_ranked_amount_parser_closes_image_it_opens(
    ...
    monkeypatch.setattr(
        "momo_ocr.features.screen_parsers.ranked_amount.open_decoded_image",
        open_image,
    )
...
def test_incident_log_parser_closes_image_it_opens(
    ...
    monkeypatch.setattr(
        "momo_ocr.features.screen_parsers.incident_log.open_decoded_image",
        open_image,
    )
