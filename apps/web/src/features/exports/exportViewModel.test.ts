...
describe("exportViewModel", () => {
  ...
  it("uses projected names and never exposes unresolved internal IDs", () => {
    const baseMatch = {
    ...
    const unresolved = toMatchCandidates([{ ...baseMatch, gameTitleId, seasonMasterId }])[0];

    expect(unset?.description).toBe("作品名未取得・シーズン名未取得");
