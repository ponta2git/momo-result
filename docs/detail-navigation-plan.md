...
恒久契約の正本は、開催を[開催一覧・詳細要求](requirements/held-event-detail.md)、試合を[基本要求](requirements/base.md#4-mvp画面)、メモを[試合メモ要求](requirements/match-note.md)、対戦順を[戦績比較要求](requirements/series-comparison.md#2-data--scope-invariants)とする。採用内容は各正本へ反映済み。本書は判断・工程・検証の記録とし、恒久仕様の二重管理先にしない。

## 2. 敵対的レビューの結論

機能の目的と既存詳細readを拡張する方向は妥当。ただし初稿は、次の反例を解消せずに実装へ渡せる状態ではなかった。以下は計画レビュー時点の静的な指摘であり、その後の実装・実行による証拠は7節と区別する。
...
**隣接順に合う2つの複合索引を追加する判断となった。** 変更前のschemaには試合の`played_at`単列indexと開催内番号の一意性があるが、今回の全tupleと開催日時＋IDに合うindexがなかった。各1件のresponseであることは、走査・sortの仕事量が一定という保証ではない。

所有repositoryの規約に従い、`momo-db`のmigration `0057_detail_navigation_indexes.sql`へ、開催の`start_at → id`と試合の`played_at → held_event_id → match_no_in_event → id`の索引を追加した。IDのcollationは境界条件・取得順と同じ`C`とした。隔離した合成fixtureで索引適用後の実行計画と性能範囲を確認済み。公開文書へ実測値や実行環境の構成を転記せず、検証結果を本番性能の保証へ拡張しない。

通常の`CREATE INDEX`によるmigrationのため、書込みとの競合で待機が生じ得る。DB expansionを先に適用してからconsumerをreleaseする依存がある。正規生成・migration・検証とconsumerのpin更新は実装工程で扱い、DB変更の取り込み・本番適用は6節のrelease境界に従う。
...
- 未保存またはメモ処理の応答待ちには試合自体の削除も停止し、理由を示す。確認dialogからの削除も抑止し、削除成功後の遷移だけがguardで止まってdetailをevictする競合を防ぐ。
...
両画面とも本文を読み終えたページ下部の一か所へ配置する。試合は結果・メモ・記録情報の後、開催は開催内容の後とする。リンクと端の非link表示は文字色・余白・整列を共通化し、状態による位置のずれを防ぐ。試合は各枠内の中央揃えとする。開催は2列表示時に前を左揃え・次を右揃え、狭幅の縦並びでは中央揃えとする。前後は「前の試合／後の試合」「前の開催／次の開催」とし、存在しない側は理由のある非link表示にする。矢印だけや`#`へのlinkにしない。
...
shared UIが所有するのは名前付きnavigation、2方向のlink/非link、keyboard/focus、hit targetと狭幅配置まで。業務語彙・URL・API・cacheはfeature/API側に残す。再取得で操作を停止するときはanchor要素を維持して古い`href`を外し、focusを失わずに移動を防ぐ。前後移動に限定する小さいfocus補助を使う。
...
以下の所有境界でA〜Eを実施した。新schema接続後の代表ブラウザ確認を含め、実装工程は完了している。
...
| V7 | メモを失わず、送信済み操作を破棄と誤認させない | data router＋遅延mutation。dirtyのキャンセル/破棄、保存/削除中のlinkとbrowser back、未保存/応答待ちの試合削除停止、成功後移動、失敗再試行、編集中refetch/409、保存成功＋GET失敗、404時の未保存本文退避 |
...
releaseには`momo-db`の索引migration `0057`を先に適用し、そのrevisionを参照するconsumerのAPI/Webを続ける依存がある。索引作成中の書込み待ちを考慮し、DB側の適用確認前にconsumerをreleaseしない。ブラウザに残る旧世代/新世代まで同一release単位で自動的に排除できるとは扱わない。具体的な配備・rollback手順やprovider構成は複製せず、[公開運用規約](ops/README.md)と承認された運用手順に従う。

通常実装PRは`develop`向けの一つの単位とし、本文に`Fixes MOM-21`と`Fixes MOM-23`を記載する。momo-db変更は所有repositoryで別途扱い、依存revisionと導入条件を揃える。release PRは[Git規約](dev-rule.md#8-git)どおり対象PR・両issue・利用者向けRelease notesを記載する。

実装完了は、要求正本・実装・生成物が一致し、V1〜V9と必要gateの証拠が揃い、重要な未検証境界を明記した時点とする。再実行は結果を無効にする変更・失敗・具体的な未解決事項の影響範囲に限る。計画作成、実装完了、release完了を別に扱う。

## 7. 今回の成果と未検証事項

要求正本・実装・生成物へ合意済みの範囲・順序・起点保持を反映した。API実装は`0b28d460`、Web実装は`2a2c5cbf`、momo-dbの索引migrationは`709423e1de054360b63f9bf57bb18f418aaa1795`で記録し、consumerのpinも後者へ更新済み。追加の利用者判断を要する仕様の不明点はない。

| 境界 | 完了した証拠 |
| --- | --- |
| API | `apiQuality`・通常test 496件・変更前schemaでのDB test 203件が成功。新schemaでもDB test 203件と一時性能診断4件が成功し、索引適用後の性能範囲を再確認 |
| momo-db | 正規のmigration生成、`build`・`db:check`、integration 9件・migration 11件が成功 |
| Web | 全163ファイル・999件が成功。後続のメモと試合削除の競合修正は関連2ファイル・29件を再検証。最終差分のformat・typecheck・buildと、影響範囲のlintが成功。contract checkも通過 |
| 実ブラウザ | 開催一覧の2ページ目への復帰、keyboard前後移動と移動先focus、作品・開催をまたぐ試合移動、現在試合の比較scope/IDと元の条件付き一覧への復帰を確認 |
| 表示・メモ保護 | 広幅/狭幅の配置・44px以上の操作領域・横overflowなしを確認。dirty時の移動取消で入力を保持し試合削除を停止。保存応答待ちは移動を待機させ、破棄を提示せず、保存完了後の隣接移動で別試合のメモを表示 |
| 新schemaとの接続 | migration適用済みの隔離DB＋実APIで、開催/試合の前後移動・端・focus・起点保持を再確認。小数秒が異なる同番号の試合/開催を区別して表示。遅延/失敗を注入した再取得でfocusを維持して古いhrefを除き、実APIの再取得成功で復帰することを確認 |
| 公開文書 | `git diff --check`と`pnpm public:safety:check`が成功 |

全test集合の成功後の修正には影響範囲の証拠を追加し、同じ集合の重複実行はしていない。Web lintの既存警告と既存のbundle size警告は今回の新規違反として扱わず、新規のlint・format違反は解消した。

2026-09-25の利用者レビューを反映し、両画面の前後移動を本文の後へ移動した。リンクと最初・最後の非link表示は文字色・余白・各枠内の中央揃えを共通化した。関連4ファイル・46test、format・lint・typecheck・buildが成功。Playwright MCPで両画面の広幅/狭幅・最初/最後の表示と下部配置、文字色と中心位置の一致、横overflowなし、keyboard移動後の見出しfocusを確認した。
