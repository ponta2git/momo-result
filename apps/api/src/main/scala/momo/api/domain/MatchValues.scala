...
object MatchNoInEvent:
  ...
  def unsafeFromInt(value: Int): MatchNoInEvent =
    fromInt(value).fold(error => sys.error(error.message), identity)
...
object PlayOrder:
  ...
  def unsafeFromInt(value: Int): PlayOrder =
    fromInt(value).fold(error => sys.error(error.message), identity)
...
object Rank:
  ...
  def unsafeFromInt(value: Int): Rank =
    fromInt(value).fold(error => sys.error(error.message), identity)
...
object IncidentCount:
  ...
  def unsafeFromInt(value: Int): IncidentCount =
    fromInt(value).fold(error => sys.error(error.message), identity)
