...
- 同一 transaction で FK 関連 row を作成・更新する repository method は、成功 path と最終的な
  linked row values を PostgreSQL integration test で検証する。非 deferrable FK は statement
  単位で検証されるため、in-memory/usecase test で代用しない。
...
- OCR worker の Docker-backed Redis/Postgres、native `tesserocr`/tessdata、複数 adapter smoke は
  ...
  で明示的に実行する。
- OCR worker の integration test は session-scoped Testcontainers を fixture で共有して起動コストを抑える。
  共有 container 内では `tests/integration/resources.py` の test-scoped suffix / fixture を使い、Redis stream/group、
  DB row id、worker id を固定値で共有しない。
