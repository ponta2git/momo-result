...
- OCR 系の `ocr_jobs.draft_id`、`ocr_drafts.job_id` には FK を張りません。参照整合は API / worker 側のトランザクションとリポジトリ実装で保証してください。
...
momo-result のログイン主体を保持する。`members` は試合参加者マスタとして残し、ログインアカウントとは任意に紐づける。
...
CREATE TABLE momo_login_accounts (
  ...
  player_member_id text REFERENCES members(id) ON DELETE RESTRICT,
...
CREATE INDEX momo_login_accounts_player_member_id_idx ON momo_login_accounts(player_member_id);
CREATE INDEX momo_login_accounts_login_enabled_idx ON momo_login_accounts(login_enabled);
CREATE INDEX momo_login_accounts_is_admin_idx ON momo_login_accounts(is_admin);
```

- `discord_user_id` は Discord OAuth の照合キー。
- `player_member_id = NULL` のアカウントは、戦績を操作できるが試合参加者ではない運用者。
- `login_enabled = false` のアカウントはログイン不可。無効化時は既存 session を削除する。
- `is_admin = true` かつ `login_enabled = true` のアカウントを最低1件維持する。
- migration `0013_login_accounts.sql` で固定4名の初期アカウントを seed 済み。初期管理者は `account_ponta`。
...
- OAuth user は `momo_login_accounts.discord_user_id`（Discord user id）で照合する。
- `app_sessions.account_id` が認証主体。`member_id` は対応プレーヤーがいる場合だけ入る互換・参照用の nullable 列。
...
CREATE TABLE app_sessions (
  id text PRIMARY KEY,
  member_id text,
  account_id text NOT NULL REFERENCES momo_login_accounts(id) ON DELETE CASCADE,
...
CREATE TABLE match_drafts (
  id text PRIMARY KEY,
  created_by_account_id text NOT NULL REFERENCES momo_login_accounts(id) ON DELETE RESTRICT,
  created_by_member_id text REFERENCES members(id) ON DELETE RESTRICT,
  status text NOT NULL,
  held_event_id text REFERENCES held_events(id) ON DELETE RESTRICT,
  match_no_in_event integer,
  game_title_id text REFERENCES game_titles(id) ON DELETE RESTRICT,
  layout_family text,
  season_master_id text REFERENCES season_masters(id) ON DELETE RESTRICT,
  owner_member_id text REFERENCES members(id) ON DELETE RESTRICT,
  map_master_id text REFERENCES map_masters(id) ON DELETE RESTRICT,
  played_at timestamptz,
  total_assets_draft_id text,
  revenue_draft_id text,
  incident_log_draft_id text,
  confirmed_match_id text REFERENCES matches(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX match_drafts_created_by_account_status_idx
  ON match_drafts(created_by_account_id, status);
```

- `created_by_account_id` は作成操作を行ったログインアカウント。
- `created_by_member_id` は互換・参照用の nullable 列。対応プレーヤーを持たない運用者が作成した場合は `NULL`。
...
CREATE INDEX matches_created_by_account_id_idx ON matches(created_by_account_id);
...
- `created_by_account_id` は確定操作を行ったログインアカウント。`created_by_member_id` は対応プレーヤーを持つアカウントだけに入る nullable 列。
...
状態変更 API の再送による副作用重複を防ぐ。ログインアカウント単位でスコープする。

```sql
CREATE TABLE idempotency_keys (
  key text NOT NULL,
  account_id text NOT NULL REFERENCES momo_login_accounts(id),
...
- `account_id` が主キー構成要素。`member_id` は互換・参照用の nullable 列。
- `response_headers` に authorization、cookie、secret を保存してはいけない。
- 期限切れレコードは `expires_at` で削除する。
...
| ログインアカウント無効化 | `momo_login_accounts.login_enabled = false` と対象 account の `app_sessions` 削除。 |
...
- **`momo_login_accounts` の seed**: momo-db `0013_login_accounts.sql` で固定4名に対応する初期ログインアカウントを seed 済み。追加 Discord ID は管理画面から `player_member_id = NULL` の運用者として作成できる。
...
- **作成者の取得元**: request body には含めず、認証 session の `AuthenticatedAccount.accountId` から `created_by_account_id` を設定する。対応プレーヤーがある場合だけ `created_by_member_id` も設定する。
- **試合の削除方式**: 修正履歴は保存しない方針と整合させ、物理削除で進める。`matches` 削除時は FK CASCADE で `match_players` / `match_incidents` も削除される。
- **CSV 出力の権限**: ログイン中の有効アカウントに許可。RLS は MVP では使用せず、API 層で session を確認する。
- **管理者アカウント保護**: `is_admin = true` かつ `login_enabled = true` のアカウントが0件になる更新は API usecase で拒否する。
