...
- ocr-worker 変更: `ruff format --check`、`ruff check`、`mypy`、`pytest`。Redis/Postgres/native OCR など外部 wire 動作に触れた場合は `pytest -m integration` も実行する。integration が skip / 未実行なら、その外部依存挙動は未検証として扱う。
