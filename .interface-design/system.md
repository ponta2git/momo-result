...
- Reserve pale surface backgrounds for visual anchors that establish the reading path: the current scope, selection, or primary result; page-local navigation; section and labeled table/matrix headers; actionable disclosure triggers; and the primary next-match hypothesis. Supporting facts, metric-definition disclosures, empty table cells, unlabeled header intersections, chart containers, result cards, explanatory copy, and expanded detail bodies stay on the base surface and use spacing, type, and quiet borders for structure.
...
- A neutral plot or per-series chart frame is not data encoding. Keep it on the base surface; tint only marks, ranges, or tracks whose color maps to a value, series, warning, or current selection.
...
- Give every visible analysis section and subsection a stable heading relationship so it is also a named navigation region for assistive technology.
- Build comparison matrices from the shared native-table primitives. Keep row and column headers semantic, put their intersection labels in the corner cell, and define unlabeled stacked cell values with an adjacent `dl`. Persistent matrix guidance is at least 11px.
- For complex indicators, keep the initial surface to the comparison point, evidence scope, quality, and result. Put meaning, judgment, next evidence, misuse boundary, and method details in that order inside a focused disclosure.
- Show parts-of-a-whole evidence such as crown certainty as one 100% composition bar plus a labeled value list, not as independent progress bars. When only one rank-signal candidate remains, say “候補はこの1件” instead of implying that 100% is a probability.
