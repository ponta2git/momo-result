...
/** Validation shared across master ID inputs.
  *
  * Master IDs become FKs in `matches.*_id` columns; we restrict them to
  * lowercase alphanumerics plus underscore so they remain stable, URL-safe,
  * and trivially comparable across summit and momo-result.
  */
private object MasterIdValidation:
  private val Pattern = "^[a-z][a-z0-9_]{1,63}$".r
  def validate(field: String, value: String): Either[AppError, String] =
...
final class CreateGameTitle[F[_]: MonadThrow](
  ...
  def run(command: CreateGameTitleCommand): F[Either[AppError, GameTitle]] =
    ...
    validated match
      ...
      case Right((id, name, lf)) =>
        titles.find(id).flatMap {
          case Some(_) =>
            MonadThrow[F].pure(Left(AppError.Conflict(s"game_title with id=$id already exists.")))
          case None =>
            for
              order <- titles.nextDisplayOrder
              created <- now
              title = GameTitle(id = id, name = name, layoutFamily = lf, displayOrder = order, createdAt = created)
...
final class CreateMapMaster[F[_]: MonadThrow](
  ...
  def run(command: CreateMapMasterCommand): F[Either[AppError, MapMaster]] =
    ...
    validated match
      case Left(err) => MonadThrow[F].pure(Left(err))
      case Right((id, gtId, name)) =>
        titles.find(gtId).flatMap {
          case None =>
            MonadThrow[F].pure(Left(AppError.NotFound("game_title", gtId)))
          case Some(_) =>
            maps.find(id).flatMap {
              case Some(_) =>
                MonadThrow[F].pure(
                  Left(AppError.Conflict(s"map_master with id=$id already exists."))
                )
              case None =>
                for
                  order <- maps.nextDisplayOrder(gtId)
                  created <- now
                  m = MapMaster(id = id, gameTitleId = gtId, name = name, displayOrder = order, createdAt = created)
                  _ <- maps.create(m)
                yield Right(m)
            }
        }
      ...
      case Right((id, gtId, name)) =>
        titles.find(gtId).flatMap {
          case None =>
            MonadThrow[F].pure(Left(AppError.NotFound("game_title", gtId)))
          case Some(_) =>
            seasons.find(id).flatMap {
              case Some(_) =>
                MonadThrow[F].pure(
                  Left(AppError.Conflict(s"season_master with id=$id already exists."))
                )
              case None =>
                for
                  order <- seasons.nextDisplayOrder(gtId)
                  created <- now
                  s = SeasonMaster(id = id, gameTitleId = gtId, name = name, displayOrder = order, createdAt = created)
                  _ <- seasons.create(s)
                yield Right(s)
            }
        }
