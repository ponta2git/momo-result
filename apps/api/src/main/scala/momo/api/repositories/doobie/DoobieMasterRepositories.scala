...
final class DoobieGameTitlesRepository[F[_]: MonadCancelThrow](xa: Transactor[F])
  ...
  override def find(id: String): F[Option[GameTitle]] =
    sql"""
      SELECT id, name, layout_family, display_order, created_at
      FROM game_titles
      WHERE id = $id
  ...
  override def create(title: GameTitle): F[Unit] =
    sql"""
      INSERT INTO game_titles (id, name, layout_family, display_order, created_at)
      VALUES (${title.id}, ${title.name}, ${title.layoutFamily}, ${title.displayOrder}, ${title.createdAt})
    """.update.run.void.transact(xa)

  override def nextDisplayOrder: F[Int] =
    sql"SELECT COALESCE(MAX(display_order), 0) + 1 FROM game_titles"
      .query[Int]
      .unique
      .transact(xa)
...
final class DoobieMapMastersRepository[F[_]: MonadCancelThrow](xa: Transactor[F])
  ...
  override def find(id: String): F[Option[MapMaster]] =
    sql"""
  ...
  override def create(map: MapMaster): F[Unit] =
    sql"""
...
final class DoobieSeasonMastersRepository[F[_]: MonadCancelThrow](xa: Transactor[F])
  ...
  override def find(id: String): F[Option[SeasonMaster]] =
    sql"""
      SELECT id, game_title_id, name, display_order, created_at
      FROM season_masters
      WHERE id = $id
  ...
  override def create(season: SeasonMaster): F[Unit] =
    sql"""
      INSERT INTO season_masters (id, game_title_id, name, display_order, created_at)
      VALUES (${season.id}, ${season.gameTitleId}, ${season.name}, ${season.displayOrder}, ${season.createdAt})
    """.update.run.void.transact(xa)
...
final class DoobieIncidentMastersRepository[F[_]: MonadCancelThrow](xa: Transactor[F])
  ...
  override def list: F[List[IncidentMaster]] =
    sql"""
...
/** `member_aliases` row.
  *
  * The Scala domain `MemberAlias` does not carry the DB-level `id`, so we
  * generate a stable id when inserting. The unique index on
  * `(member_id, alias)` keeps duplicates idempotent via ON CONFLICT.
  */
