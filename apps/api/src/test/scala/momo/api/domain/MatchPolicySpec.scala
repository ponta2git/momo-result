...
final class MatchPolicySpec extends FunSuite:
  ...
  test("multiple errors accumulate (owner not allowed + matchNo invalid + bad players + raw field errors)"):
    val bad = happyInput.copy(
      matchNoInEvent = 0,
      ownerMemberId = MemberId.unsafeFromString("intruder"),
      players = happyInput.players.take(3) :+ MatchFixtures
        .playerInput("d", playOrder = 5, rank = 0)
  ...
  test("toMessage joins domain errors for application boundary"):
    val bad = happyInput.copy(matchNoInEvent = 0, ownerMemberId = MemberId.unsafeFromString("intruder"))
