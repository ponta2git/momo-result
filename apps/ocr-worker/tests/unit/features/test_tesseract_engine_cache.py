...
from momo_ocr.features.text_recognition import tesseract as tesseract_module
...
def test_init_resolves_executable_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tesseract_module.shutil, "which", fake_which)
...
def test_recognize_does_not_call_shutil_which(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tesseract_module.shutil, "which", lambda _name: "/bin/tesseract")
    engine = TesseractEngine()
