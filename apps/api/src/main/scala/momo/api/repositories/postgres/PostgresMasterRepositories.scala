...
final class PostgresGameTitlesRepository[F[_]: MonadCancelThrow](transactor: Transactor[F])
  ...
  override def find(id: String): F[Option[GameTitle]] = sql"""
      ...
      FROM game_titles
      WHERE id = $id
    """.query[GameTitle].option.transact(transactor)
...
final class PostgresMapMastersRepository[F[_]: MonadCancelThrow](transactor: Transactor[F])
  ...
  override def find(id: String): F[Option[MapMaster]] = sql"""
      ...
      FROM map_masters
      WHERE id = $id
    """.query[MapMaster].option.transact(transactor)
...
final class PostgresSeasonMastersRepository[F[_]: MonadCancelThrow](transactor: Transactor[F])
  ...
  override def find(id: String): F[Option[SeasonMaster]] = sql"""
      ...
      FROM season_masters
      WHERE id = $id
    """.query[SeasonMaster].option.transact(transactor)
...
final class PostgresIncidentMastersRepository[F[_]: MonadCancelThrow](transactor: Transactor[F])
  ...
  override def list: F[List[IncidentMaster]] = sql"""
    ...
    """.query[IncidentMaster].to[List].transact(transactor)
...
final class PostgresMemberAliasesRepository[F[_]: MonadCancelThrow](
  ...
  override def list(memberId: Option[String]): F[List[MemberAlias]] =
    ...
    (base ++ where ++ order).query[MemberAlias].to[List].transact(transactor)
