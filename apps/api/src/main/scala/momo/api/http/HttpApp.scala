...
import momo.api.endpoints.{
  AuthEndpoints, AuthMeResponse, CancelOcrJobResponse, ConfirmMatchResponse, CreateOcrJobResponse,
...
import momo.api.repositories.doobie.{
  DoobieGameTitlesRepository, DoobieHeldEventsRepository, DoobieIncidentMastersRepository,
  DoobieMapMastersRepository, DoobieMatchesRepository, DoobieOcrDraftsRepository,
  DoobieOcrJobsRepository, DoobieSeasonMastersRepository,
}
...
object HttpApp:
  /**
   * Test-only handle so specs can seed master tables without exposing real HTTP master endpoints
   * (those land in section D).
   */
  ...
  /**
   * Build all dependencies. When `config.database` is set we use Doobie repositories backed by
  ...
  def wired[F[_]: Async](config: AppConfig): Resource[F, Wired[F]] = config.database match
    case Some(db) => Database.transactor[F](db).evalMap { xa =>
        for queue <- InMemoryQueueProducer.create[F] yield
          ...
          val seasonMasters: SeasonMastersRepository[F] = DoobieSeasonMastersRepository[F](xa)
          val incidentMasters: IncidentMastersRepository[F] = DoobieIncidentMastersRepository[F](xa)
          assemble(
            config = config,
            queue = queue,
            jobs = jobs,
  ...
  private def build[F[_]: Async](
    ...
    ): F[Either[ProblemDetails.ErrorInfo, momo.api.auth.AuthenticatedMember]] = devUser match
    ...
    val healthRoutes = Http4sServerInterpreter[F]().toRoutes(List[ServerEndpoint[Any, F]](
      ...
      AuthEndpoints.me.serverLogic { devUser =>
        authenticate(devUser)
          .map(_.map(member => AuthMeResponse(member.memberId.value, member.displayName)))
      },
      UploadEndpoints.uploadImage.serverLogic { case (devUser, csrfToken, parts) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => extractFile(parts) match
              case Left(error) => Async[F].pure(Left(toProblem(error)))
              case Right((fileName, contentType, bytes)) => uploadImage
                  .run(fileName, contentType, bytes).map {
      ...
      OcrJobEndpoints.get.serverLogic { case (jobId, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => getOcrJob.run(jobId).map {
      ...
      OcrJobEndpoints.cancel.serverLogic { case (jobId, devUser, csrfToken) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => cancelOcrJob.run(jobId).map {
      ...
      OcrDraftEndpoints.get.serverLogic { case (draftId, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => getOcrDraft.run(draftId).map {
      ...
      OcrDraftEndpoints.listByIds.serverLogic { case (ids, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => getOcrDraftsBulk.run(ids).map {
              case Right(items) => Right(OcrDraftListResponse(items.map(OcrDraftResponse.from)))
              case Left(error) => Left(toProblem(error))
            }
        }
      },
      HeldEventsEndpoints.list.serverLogic { case (q, limit, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => listHeldEvents.run(q, limit).map { items =>
      ...
      HeldEventsEndpoints.create.serverLogic { case (devUser, csrfToken, request) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => createHeldEvent.run(CreateHeldEventCommand(request.heldAt)).map {
      ...
      MatchesEndpoints.confirm.serverLogic { case (devUser, csrfToken, request) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(member) => confirmMatch.run(request, member.memberId).map {
              case Right(record) => Right(ConfirmMatchResponse(
                  matchId = record.id,
      ...
      GameTitlesEndpoints.create.serverLogic { case (devUser, csrfToken, request) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => createGameTitle
              .run(CreateGameTitleCommand(request.id, request.name, request.layoutFamily)).map {
      ...
      MapMastersEndpoints.list.serverLogic { case (gameTitleId, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => mapMasters.list(gameTitleId)
      ...
      MapMastersEndpoints.create.serverLogic { case (devUser, csrfToken, request) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => createMapMaster
              .run(CreateMapMasterCommand(request.id, request.gameTitleId, request.name)).map {
      ...
      SeasonMastersEndpoints.list.serverLogic { case (gameTitleId, devUser) =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => seasonMasters.list(gameTitleId)
      ...
      SeasonMastersEndpoints.create.serverLogic { case (devUser, csrfToken, request) =>
        authorizeMutation(devUser, csrfToken).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => createSeasonMaster
              .run(CreateSeasonMasterCommand(request.id, request.gameTitleId, request.name)).map {
      ...
      IncidentMastersEndpoints.list.serverLogic { devUser =>
        authenticate(devUser).flatMap {
          case Left(error) => Async[F].pure(Left(error))
          case Right(_) => incidentMasters.list.map(items =>
