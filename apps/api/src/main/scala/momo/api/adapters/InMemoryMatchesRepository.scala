...
final class InMemoryMatchesRepository[F[_]: Sync] private (ref: Ref[F, Map[String, MatchRecord]])
  ...
  override def countByHeldEvents(heldEventIds: List[String]): F[Map[String, Int]] = ref.get
    .map { m =>
...
object InMemoryMatchesRepository:
  def create[F[_]: Sync]: F[InMemoryMatchesRepository[F]] = Ref
    .of[F, Map[String, MatchRecord]](Map.empty).map(new InMemoryMatchesRepository(_))
