...
final class InMemoryOcrSubmissionsRepository[F[_]: Async] private (
    ...
    mutex: Semaphore[F],
    drafts: InMemoryMatchDraftsRepository[F],
  ...
  override def put(submission: OcrSubmission): F[Either[AppError, OcrSubmission]] =
    mutex.permit.use { _ =>
      state.get.flatMap { saved =>
        saved.get(submission.id) match
          case Some(existing) =>
          ...
          case None => drafts.find(submission.matchDraftId).flatMap {
              case Some(draft)
                ...
                if saved.values.count(s =>
                    s.ownerAccountId == submission.ownerAccountId && s.status == "open"
                  ) >= OcrSubmissions.OpenLimit
                then
                  Left(AppError.TooManyRequests("Too many unfinished OCR submissions.")).pure[F]
                else
                  (state.update(_ + (submission.id -> submission)) *>
                    drafts.startEmptySubmission(
                      submission.matchDraftId,
                      submission.createdAt
                    )).as(Right(submission))
              case None =>
                Left(AppError.NotFound("match draft", submission.matchDraftId.value)).pure[F]
  ...
  override def failAdmission(
    ...
    mutex.permit.use(_ =>
      state.update(_.map { case (id, submission) =>
        id ->
          (if submission.ownerAccountId == owner && submission.status == "open" then
             submission.copy(members = submission.members.map { member =>
               if member.status == "pending" && member.uploadIdempotencyKeyHash == keyHash &&
                 member.imageSha256 == sha256 && member.imageByteLength == byteLength
               then
  ...
  private[inmemory] def register(id: String, screen: ScreenType, job: OcrJobId): F[Unit] =
    state.update(_.updatedWith(id)(_.map(s =>
...
object InMemoryOcrSubmissionsRepository:
  def create[F[_]: Async](drafts: InMemoryMatchDraftsRepository[F])
      : F[InMemoryOcrSubmissionsRepository[F]] =
