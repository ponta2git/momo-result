...
class AppConfigSpec extends FunSuite:
  ...
  test("toJdbcUrl: converts postgres:// URL and extracts credentials") {
    val (url, user, pass) =
      AppConfig.toJdbcUrl("postgres://summit:summit@localhost:5433/summit")
  ...
  test("toJdbcUrl: converts postgresql:// URL") {
    val (url, user, pass) =
      AppConfig.toJdbcUrl("postgresql://user:secret@db.example.com/mydb")
  ...
  test("toJdbcUrl: handles URL with query params (e.g. sslmode=require)") {
    val (url, user, pass) =
      AppConfig.toJdbcUrl("postgres://u:p@host/db?sslmode=require")
  ...
  test("toJdbcUrl: handles URL without credentials") {
    val (url, user, pass) =
      AppConfig.toJdbcUrl("postgres://localhost:5432/summit")
