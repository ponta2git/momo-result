...
final class IdempotencyIntegrationSpec extends MomoCatsEffectSuite with HttpAppTestFixtures:
  ...
  test("idempotency: in-flight same key returns a specific 409 code") {
    ...
    yield second match
      case Left(problem) =>
        assertEquals(problem.status, sttp.model.StatusCode.Conflict)
        assertEquals(problem.body.code, "IDEMPOTENCY_IN_PROGRESS")
  ...
  test("idempotency: undecodable stored replay returns an internal problem") {
    ...
    yield
      ...
      second match
        case Left(problem) =>
          assertEquals(problem.status, sttp.model.StatusCode.InternalServerError)
          assertEquals(problem.body.code, "INTERNAL_ERROR")
