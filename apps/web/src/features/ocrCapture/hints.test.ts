...
describe("buildOcrHints", () => {
  ...
  it("builds known player aliases for the fixed four members", () => {
    ...
    expect(aliases.map((alias) => alias.memberId)).toEqual(["ponta", "akane-mami", "otaka", "eu"]);
    expect(aliases.find((alias) => alias.memberId === "akane-mami")?.aliases).toContain("NO11社長");
    expect(aliases.find((alias) => alias.memberId === "otaka")?.aliases).toContain("オータカ社長");
  });
