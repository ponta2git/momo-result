...
object OcrJobEndpoints:
  ...
  val create: PublicEndpoint[CreateInput, ErrorInfo, CreateOcrJobResponse, Any] =
    endpoint.post
      .in("api" / "ocr-jobs")
  ...
  val get: PublicEndpoint[(String, Option[String]), ErrorInfo, OcrJobResponse, Any] =
    endpoint.get
  ...
  val cancel: PublicEndpoint[CancelInput, ErrorInfo, CancelOcrJobResponse, Any] =
    endpoint.delete
      .in("api" / "ocr-jobs" / path[String]("jobId"))
      .in(header[Option[String]]("X-Dev-User"))
      .in(header[Option[String]]("X-CSRF-Token"))
