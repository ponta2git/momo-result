...
final class NotificationSettingsHttpSpec extends MomoCatsEffectSuite with HttpAppTestFixtures:
  ...
  private def request(ocr: Boolean, analysis: Boolean, ocrGeneration: String): Json =
    request(ocr, analysis, ocrGeneration, "0")
  ...
  private def request(
    ...
    Json.obj(
      "ocrCompleted" -> Json.obj(
        "enabled" -> Json.fromBoolean(ocr),
        "expectedGeneration" -> Json.fromString(ocrGeneration)
      ),
      "analysisCompleted" -> Json.obj(
        "enabled" -> Json.fromBoolean(analysis),
        "expectedGeneration" -> Json.fromString(analysisGeneration)
      ),
    )

  app.test(
    "only administrators can read and save global notification settings; writes require CSRF"
  ) { http =>
    for
      ...
      writer <-
        http.run(writeRequest(Method.PUT, uri, "account_eu").withEntity(request(false, false)))
  ...
  app.test("save returns both confirmed settings, persists on the next read, and safely replays") {
    http =>
      val write = writeRequest(
        Method.PUT,
        uri,
        Some("notification-settings-save")
      ).withEntity(request(false, true))
      for
      ...
      yield
        ...
        assertEquals(
          ...
          Json.obj(
            "ocrCompleted" ->
              Json.obj("enabled" -> Json.False, "generation" -> Json.fromString("1")),
            "analysisCompleted" ->
              Json.obj("enabled" -> Json.True, "generation" -> Json.fromString("0")),
          )
        )
  ...
  app.test("a stale expected generation rejects the entire form without changing either setting") {
    http =>
      for
  ...
  app.test("invalid generation and incomplete or incorrectly typed forms cannot save settings") {
    http =>
      val invalid = List("-1", "01", "1.0", "9223372036854775808").map(generation =>
        request(false, false, generation)
      ) ++ List(
        Json.obj("ocrCompleted" ->
          Json.obj("enabled" -> Json.False, "expectedGeneration" -> Json.fromString("0"))),
        request(false, false).mapObject(_.add(
          "ocrCompleted",
          Json.obj(
            "enabled" -> Json.fromString("false"),
            "expectedGeneration" -> Json.fromString("0")
          )
        )),
      )
      for
        before <- http.run(readGet(uri)).flatMap(_.as[Json])
        statuses <- invalid.traverse(body =>
          http.run(writeRequest(Method.PUT, uri).withEntity(body)).map(_.status)
        )
        after <- http.run(readGet(uri)).flatMap(_.as[Json])
      yield
