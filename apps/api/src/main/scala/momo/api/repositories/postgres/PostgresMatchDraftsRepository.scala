...
final class PostgresMatchDraftsRepository[F[_]: MonadCancelThrow](transactor: Transactor[F])
  ...
  override def create(draft: MatchDraft): F[Unit] = sql"""
    INSERT INTO match_drafts (
      ...
      ${draft.sourceImagesRetainedUntil}, ${draft.sourceImagesDeletedAt}, ${draft.confirmedMatchId},
      ${draft.createdAt}, ${draft.updatedAt}
    )
  """.update.run.void.transact(transactor)

  override def update(draft: MatchDraft, updatedAt: Instant): F[Boolean] = sql"""
