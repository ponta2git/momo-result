...
- 前後の試合へのlinkも同じ保護を通す。保存・メモ削除の応答待ちは未送信の変更と区別し、アプリ内移動とbrowser backを結果が分かるまで保護する。「破棄して移動」で送信済み操作を取り消せるとは説明しない。reload・タブ終了はブラウザが許す警告範囲とし、送信取消を保証しない。
- 編集開始時の本文とversionを比較基準として保持し、同じ試合の再取得で未保存本文や保存のexpectedVersionを勝手に更新しない。競合解消で利用者が明示的に最新版を採用するときだけ比較基準を更新する。最新版の取得失敗時に古いcacheを最新版と表示しない。
- 保存成功の確定と表示再取得を区別する。確定した正規化本文または削除、version、更新情報を反映し、その後の再取得失敗を保存失敗として再送させない。表示再取得だけの待機では移動を禁止し続けず、遅い保存応答でより新しいversionを上書きしない。
- 試合の不存在が確定した場合は結果や保存済みメモを復活させず、入力中の未保存本文だけを同じ試合画面内でコピー・明示破棄できるようにする。移動guardを保ち、削除済み試合への保存や孤立メモの作成は許可しない。
...
分析完了通知に載せるメモは、公開処理末尾の一括取得時点の保存済み全文で固定する。その後の保存で本文を書き換えず、取得後からcommitまでの編集を必ず含めるための追加排他は行わない。未入力メモ欄は省略し、メモ専用mutationから分析・通知を発生させない。通知全体の契約は [分析batch](series-analysis-batch.md#分析完了通知) を参照する。
