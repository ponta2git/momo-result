...
- 通常のruntime releaseは、`develop`上の選択したsnapshotを指す`release/*`から`master`へのPRで行う。最新baseへの追従が必要な場合は、選択したsnapshotのtreeを変えない現在の`master`との同期mergeだけを許可する。`master`へのmergeがdeployを開始し、post-deploy verification成功後に同じcommitを指すGitHub Releaseを発行する。
- 通常PRは`develop`へsquash mergeし、`master`へ直接pushしない。release branchへ独自の内容変更を加えず、release PR作成後の`develop`変更を暗黙に取り込まない。
