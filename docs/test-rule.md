...
| ocr-worker | `uv run ruff format --check .`, `uv run ruff check .`, `uv run mypy`, `uv run pytest`, 外部依存変更では `uv run pytest -m integration` |
