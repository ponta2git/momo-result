...
import momo.api.domain.{NotificationGeneration, NotificationSettingUpdate, NotificationSettingsUpdate}
import momo.api.endpoints.{NotificationSettingUpdateRequest, NotificationSettingsEndpoints, NotificationSettingsResponse, NotificationSettingsUpdateRequest}
...
object NotificationSettingsModule:
    ...
    SecuredEndpoint.adminMutationLogic(security, NotificationSettingsEndpoints.update) { account =>
      { case (key, request) =>
        IdempotencyReplay.wrap[F, NotificationSettingsUpdateRequest, NotificationSettingsResponse](
          idempotency, key, account, HttpOperation.UpdateNotificationSettings, request, nowF,
  ...
  private def toCommand(request: NotificationSettingsUpdateRequest): Either[AppError, NotificationSettingsUpdate] =
    def setting(value: NotificationSettingUpdateRequest): Either[AppError, NotificationSettingUpdate] =
    ...
    (setting(request.ocrCompleted), setting(request.analysisCompleted)).mapN(NotificationSettingsUpdate.apply)
