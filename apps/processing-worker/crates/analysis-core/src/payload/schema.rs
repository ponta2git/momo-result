...
const OWNER_AVERAGE: Schema = Schema::Object(&[field("average", &NULLABLE_NUMBER)]);
const OWNER_TOTAL: Schema = Schema::Object(&[
    field("count", &OWNER_COUNT),
    field("average", &NULLABLE_NUMBER),
]);
const OWNER_RANK_ENTRY: Schema = Schema::Object(&[
    field("rank", &RANK),
    field("count", &COUNT),
    field("rate", &NULLABLE_RATE),
]);
const OWNER_RANK: Schema = Schema::Object(&[
    field(
        "average",
        &Schema::Nullable(&Schema::NumberRange {
            minimum: 1.0,
            maximum: 4.0,
        }),
    ),
    field(
        "distribution",
        &Schema::Array {
            item: &OWNER_RANK_ENTRY,
            maximum: 4,
        },
    ),
]);
const OWNER_GINJI: Schema = Schema::Object(&[
    field("count", &OWNER_COUNT),
    field("average", &NULLABLE_NUMBER),
    field("encounterMatches", &COUNT),
    field("encounterRate", &NULLABLE_RATE),
]);
const OWNER_CELL: Schema = Schema::Object(&[
    field("ownerMemberId", &ID),
    field("rank", &OWNER_RANK),
    field("assets", &OWNER_AVERAGE),
    field("revenue", &OWNER_AVERAGE),
    field("destination", &OWNER_TOTAL),
    field("ginji", &OWNER_GINJI),
]);
const OWNER_HEADER: Schema = Schema::Object(&[
    field("memberId", &ID),
    field("targetCount", &COUNT),
    field("qualityStatus", &QUALITY),
]);
const OWNER_ROW: Schema = Schema::Object(&[
    field("memberId", &ID),
    field(
        "cells",
        &Schema::Array {
...
const OWNER_COMPARISON: Schema = Schema::Object(&[
    field(
        "owners",
        &Schema::Array {
            item: &OWNER_HEADER,
...
mod json_schema_export_tests {
    ...
    fn publication_contract_document() -> GeneratedSchema {
        ...
        contents.push('\n');
        GeneratedSchema {
            file_name: "series-analysis-publication-contract-v2.json",
            contents,
        }
    }
    ...
    fn shared_normal_fixtures_and_invalid_mutations_follow_owner_descriptor() {
        ...
        for version in [1, 2, 3, 4] {
            let mut obsolete = aggregate.clone();
            *obsolete
                .get_mut("schemaVersion")
                .unwrap_or_else(|| panic!("fixture version is required")) = json!(version);
            assert!(validate_aggregate(&obsolete).is_err());
        }
        for version in [1, 2, 3] {
            let mut obsolete = review.clone();
            *obsolete
                .get_mut("schemaVersion")
                .unwrap_or_else(|| panic!("fixture version is required")) = json!(version);
            assert!(validate_review(&obsolete).is_err());
        }
